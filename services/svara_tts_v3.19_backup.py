...
# Chunk size parameters (characters).  Matches Sarvam's official
# svara-longform-generation notebook: target=150, max=250, min=50.
# Previous values (120/140) were too aggressive and combined with
# max_tokens=350 caused speech truncation at the 4.01s audio cap.
# With max_tokens=2000 there's ample room for 250-char chunks.
_CHUNK_TARGET = 150
_CHUNK_MAX = 250
_CHUNK_MIN = 50
...
# Sarvam notebook uses 0.055s; matching that value.
_CROSSFADE_SEC = 0.055
...
class SvaraTTSConfig:
    ...
    top_p: float = 0.9
    max_tokens: int = 2000  # Sarvam notebook uses 4500; 2000 is ample for voice-mode sentences
...
class SvaraTTSService(TTSService):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Pre-connected WebSocket pool for eliminating per-sentence connect overhead.
        # _warm_ws holds a pre-established, ready-to-use WebSocket connection.
    ...
    async def start(self, frame: StartFrame):
        ...
        # Pre-warm a WebSocket connection so the first sentence has zero connect overhead
        self._start_warming()
    ...
    # ── Pre-warm WebSocket pool ─────────────────────────────────────
    # Opens a WebSocket to Svara in the background so the next run_tts
    # call can skip the ~290ms connect+ack overhead.

    def _start_warming(self):
        """Kick off a background task to pre-connect a Svara WebSocket."""
        if self._warming_task and not self._warming_task.done():
            return  # already warming
        self._warming_task = asyncio.create_task(self._warm_connect())

    async def _warm_connect(self):
        """Establish a WebSocket connection to Svara and hold it ready."""
        ...
        try:
            ...
            self._warm_ws = ws
            logger.info(f"[SMOOTH] Svara WS pre-warmed in {ms:.0f}ms")
        except Exception as e:
            logger.debug(f"[SMOOTH] Svara WS pre-warm failed (non-fatal): {e}")
            self._warm_ws = None

    async def _take_warm_ws(self):
        """Return the pre-warmed WebSocket if available, else None."""
        # Wait for an in-flight warming task — give it enough time to
        # complete the connect (~200ms).  This is still faster than a
        # fresh connect+ack (~300ms) because we skip the ack wait.
        has_task = self._warming_task is not None
        task_done = self._warming_task.done() if has_task else None
        has_ws = self._warm_ws is not None
        logger.info(
            f"[SMOOTH] _take_warm_ws: task={has_task} done={task_done} ws={has_ws}"
        )
        if self._warming_task and not self._warming_task.done():
            try:
                logger.info("[SMOOTH] _take_warm_ws: waiting for warming task...")
                await asyncio.wait_for(self._warming_task, timeout=1.0)
                logger.info("[SMOOTH] _take_warm_ws: warming task completed")
            except asyncio.TimeoutError:
                logger.info("[SMOOTH] _take_warm_ws: warming task timed out")
            except Exception as e:
                logger.info(f"[SMOOTH] _take_warm_ws: warming task error: {e}")
        ws = self._warm_ws
        self._warm_ws = None
        ...
        if ws:
            try:
                ...
                if ws.close_code is not None:
                    logger.info("[SMOOTH] _take_warm_ws: warm WS already closed")
                    ws = None
            except Exception:
                ws = None
        logger.info(f"[SMOOTH] _take_warm_ws: returning ws={ws is not None}")
        return ws

    async def _close_warm_ws(self):
        """Close pre-warmed WebSocket and cancel warming task."""
        if self._warming_task and not self._warming_task.done():
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        if isinstance(frame, StartInterruptionFrame):
            ...
            # Close pre-warmed connection too
            await self._close_warm_ws()
    ...
    def _trim_silence(
        ...
        min_pad_ms: float = 5.0,
    ) -> bytes:
        """Trim leading and trailing silence from PCM16 audio.

        Adapted from Sarvam's notebook ``trim_silence_strong``.
        Uses a simple amplitude threshold to find the first/last audible
        sample and trims everything outside, keeping a small pad.

        Args:
            pcm: PCM16 little-endian audio bytes
            sample_rate: Audio sample rate
            threshold_db: Amplitude threshold in dB (samples below this are "silence")
            min_pad_ms: Milliseconds of padding to keep around speech
        """
        if not pcm or len(pcm) < 4:
            return pcm

        arr = np.frombuffer(pcm, dtype=np.int16).astype(np.float32) / 32768.0
        amp_thresh = 10 ** (threshold_db / 20)
        mask = np.abs(arr) > amp_thresh

        if not np.any(mask):
            return pcm  # All silence — return as-is

        first = int(np.argmax(mask))
        last = len(mask) - int(np.argmax(mask[::-1]))

        pad = int(sample_rate * min_pad_ms / 1000)
        start = max(0, first - pad)
        end = min(len(arr), last + pad)

        trimmed = arr[start:end]
        # Convert back to PCM16
        trimmed_int16 = np.clip(trimmed * 32768.0, -32768, 32767).astype(np.int16)
    ...
    async def run_tts(self, text: str) -> AsyncGenerator[Frame, None]:
        ...
        try:
            ...
            for i, chunk in enumerate(chunks):
                ...
                # Trim silence from each chunk (matches Sarvam notebook's
                # trim_silence_strong applied per-segment before crossfade)
                chunk_pcm = self._trim_silence(
                    chunk_pcm,
                    sample_rate=self._sample_rate or 24000,
                )

                if not chunk_pcm or len(chunk_pcm) < 4:
                    continue
    ...
    async def _run_streaming_tts_websocket(self, text: str) -> AsyncGenerator[Frame, None]:
        ...
        Uses a pre-warmed WebSocket when available to eliminate the ~290ms
        connect+ack overhead.  After receiving the "done" message, immediately
        starts warming the next connection in the background so it's ready
        ...
        try:
            # Try to reuse a pre-warmed WebSocket (saves ~290ms)
            warm_ws = await self._take_warm_ws()
            if warm_ws:
                self._current_websocket = warm_ws
                connect_ms = (_time.monotonic() - t0) * 1000
                logger.info(f"[SMOOTH] Svara WS reused pre-warmed ({connect_ms:.0f}ms)")
            else:
                # No warm connection — connect fresh
                extra_headers = self._auth_headers()
                ...
                connect_ms = (_time.monotonic() - t0) * 1000
                logger.info(f"[SMOOTH] Svara WS connect (fresh): {connect_ms:.0f}ms")
                ...
                try:
                    ...
                    if isinstance(msg, bytes):
                        ...
                        if msg_type == "done":
                            ...
                            # Start warming IMMEDIATELY on "done" so the
                            # next sentence has a pre-warmed connection ready.
                            # This gives ~200ms head-start vs doing it in finally.
            ...
            if self._current_websocket:
                try:
                    ...
                    pass
                self._current_websocket = None
            # Note: _start_warming() is called in the "done" handler above
            # so the next connection is already being established by the time
            # we reach this finally block.  No need to call it again.
